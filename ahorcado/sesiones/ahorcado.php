	...
	function jugar(){
		...
		if ($primeraVez) {
			...
			if(isset($_GET["letra"])){
				...
				$vidas =  $SESION["vidas"];
				$palabra = descesar($_SESION['palabra']);
				$letrasAdivinadas = $_SESION["letrasAdivinadas"];
				$letrasIntroducidas = $_SESION["letrasIntroducidas"];
		        if(validarLetra($l)){
...
//	echo "\t\t<input type=\"hidden\" name=\"letrasIntroducidas\" value=\"".$letrasIntroducidas."\">\n";
			$_SESION["vidas"] = $vidas;
			$_SESION["palabra"] = $palabra;
			$_SESION["letrasAdivinadas"] = $letrasAdivinadas;
			$_SESION["letrasIntroducidas"] = $letrasIntroducidas;
?>
